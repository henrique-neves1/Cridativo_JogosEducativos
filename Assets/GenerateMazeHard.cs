...
public class GenerateMazeHard : MonoBehaviour
{
    [SerializeField]
    ...
    public MazeEndControllerHard yourMazeEndControllerReference;
    ...
    public void CreateMaze()
    {
        ...
        PlayerControllerHard pc = playerInstance.GetComponent<PlayerControllerHard>();
        if (pc != null)
        {
        ...
        ExitTriggerHard invisibleTrigger = exitTriggerGO.AddComponent<ExitTriggerHard>();
    ...
    string savePath => Path.Combine(Application.persistentDataPath, "maze_save_hard.json");

    public void SaveMazeState(Vector2Int playerPos)
    {
        MazeSaveDataHard saveData = new MazeSaveDataHard();
        saveData.numX = numX;
        saveData.numY = numY;
        saveData.playerX = playerPos.x;
        saveData.playerY = playerPos.y;

        var roomList = new List<MazeSaveDataHard.RoomData>();
        ...
        for (int i = 0; i < numX; ++i)
        {
            ...
            {
                Room r = rooms[i, j];
                var rd = new MazeSaveDataHard.RoomData
                {
    ...
    public bool LoadMazeState()
    {
        ...
        MazeSaveDataHard saveData = JsonUtility.FromJson<MazeSaveDataHard>(json);
