...
public class RoomController : MonoBehaviour
{
    ...
    public void RemoveWall(string wall)
    {
        ...
        {
            case "Top":
                if (topWall) topWall.SetActive(false);
                break;
            case "Bottom":
                if (bottomWall) bottomWall.SetActive(false);
                break;
            case "Left":
    ...
    public void RemoveRandomOuterWall(int maxRows, int maxCols)
    {
        List<string> outerWalls = new List<string>();

        if (gridY == 0) outerWalls.Add("Top");
        if (gridY == maxRows - 1) outerWalls.Add("Bottom");
        if (gridX == 0) outerWalls.Add("Left");
        if (gridX == maxCols - 1) outerWalls.Add("Right");
